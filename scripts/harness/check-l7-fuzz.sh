...
./internal/l7/adapter/provider FuzzParseTerminal 10000x
...
test "$(printf '%s\n' "$expected_plan" | awk '$1 == "./internal/l7/adapter/provider" && $2 == "FuzzParseTerminal" && $3 == "10000x" {count++} END {print count + 0}')" -eq 1 || fail 'provider terminal fuzz budget changed'
test "$(printf '%s\n' "$expected_plan" | awk '$2 != "FuzzStrictConfigurationDecode" && !($1 == "./internal/l7/adapter/provider" && $2 == "FuzzParseTerminal") && $3 == "5s" {count++} END {print count + 0}')" -eq 6 || fail 'unaffected fuzz budgets changed'
...
printf 'check-l7-fuzz: PASS (8 fixed targets; strict configuration and provider terminal 10000x; 6 targets 5s each; pinned offline toolchain; disposable corpus root)\n'
