...
var expectedOwnership = map[string]ownershipExpectation{
	...
	"foundation-rebaseline-candidate":          {"exact", foundationCandidatePath, "foundation-integrator", "independent-auditor", "owner+exact-gate-2-digest"},
	"foundation-rebaseline-contract":           {"exact", "docs/artifacts/foundation-rebaseline-change-contract.md", "foundation-integrator", "owner-review", "owner+exact-gate-2-digest"},
	"foundation-rebaseline-design":             {"exact", "docs/artifacts/foundation-rebaseline-design.md", "foundation-integrator", "owner-review", "owner+exact-gate-2-digest"},
	"foundation-rebaseline-history":            {"exact", "docs/artifacts/foundation-rebaseline-history.md", "foundation-integrator", "independent-auditor", "owner+scope-audit"},
	"foundation-rebaseline-specification":      {"exact", "docs/artifacts/foundation-rebaseline-specification.md", "foundation-integrator", "owner-review", "owner+exact-gate-2-digest"},
...
var orchestrationClassForControl = map[string]string{
	...
	"readme": "wave-records", "release-allocation": "wave-records", "wave-approval": "wave-records", "wave-audit": "wave-records", "wave-audit-remediation": "wave-records", "wave-candidate": "wave-records", "wave-contract": "wave-records", "wave-design": "wave-records", "wave-design-amendment": "wave-records", "wave-evidence": "wave-records", "wave-grant-amendment": "wave-records", "wave-module-decision": "wave-records", "wave-specification": "wave-records", "wave-02-approval": "wave-records", "wave-02-audit": "wave-records", "wave-02-candidate": "wave-records", "wave-02-contract": "wave-records", "wave-02-design": "wave-records", "wave-02-evidence": "wave-records", "wave-02-specification": "wave-records", "concept-brief": "wave-records", "concept-discovery": "wave-records", "concept-rebaseline-approval": "wave-records", "concept-rebaseline-contract": "wave-records", "concept-rebaseline-design": "wave-records", "concept-rebaseline-specification": "wave-records", "foundation-rebaseline-admission-audit": "wave-records", "foundation-rebaseline-admission-evidence": "wave-records", "foundation-rebaseline-approval": "wave-records", "foundation-rebaseline-candidate": "wave-records", "foundation-rebaseline-contract": "wave-records", "foundation-rebaseline-design": "wave-records", "foundation-rebaseline-history": "wave-records", "foundation-rebaseline-specification": "wave-records",
...
func checkOwnership(root string) (int, []finding) {
	...
	if phase.phase == "foundation-rebaseline" {
		...
		findings = appendFindings(findings, crossCheckFoundationPathOwnership(pathRows, rules)...)
	} else {
...
func crossCheckFoundationPathOwnership(pathRows []tsvRow, rules map[string]ownershipExpectation) []finding {
	var findings []finding
	for _, row := range pathRows {
		relative := row["path"]
		var matches []ownershipExpectation
		...
		admissionPath := false
		for _, window := range strings.Split(row["window"], ",") {
			admissionPath = admissionPath || window == "admission"
		}
		if len(matches) == 0 {
			if admissionPath {
				findings = appendFindings(findings, newFinding("OWN-410", relative, "admission path has no shared-control ownership rule", "assign one disjoint accountable admission writer"))
			}
			continue
		}
		if len(matches) != 1 {
		...
		if matches[0].writer != row["owner"] && (admissionPath || matches[0].pathKind == "exact") {
			findings = appendFindings(findings, newFinding("OWN-411", relative, "path-policy owner differs from shared-control writer", "restore one consistent writer"))
		}
	}
	return findings
