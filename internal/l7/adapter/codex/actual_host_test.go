...
func TestActualHostInterfaceBinding(t *testing.T) {
	...
	authorizedExecutable := requiredActualEnvironment(t, "L7_AUTHORIZE_ACTUAL_CODEX_EXECUTABLE")
	expectedExecutable, err := filepath.EvalSymlinks(authorizedExecutable)
	if err != nil || expectedExecutable != authorizedExecutable || !filepath.IsAbs(expectedExecutable) {
	...
	t.Cleanup(func() { assertActualCodexSource(t, sourceRoot, expectedCandidate, expectedTree) })
	t.Cleanup(func() { assertActualCodexExecutable(t, expectedExecutable, expectedDigest) })
	adapter := New()
	...
	for _, role := range []domain.ProviderRole{domain.RoleImplementer, domain.RoleReviewer} {
		...
		if err != nil {
			findings = append(findings, fmt.Errorf("%s schema preparation: %w", role, err))
			continue
		}
		...
		if err := errors.Join(helpErr, cleanup()); err != nil {
			findings = append(findings, fmt.Errorf("%s interface contract: %w", role, err))
		}
	}
...
func assertActualCodexExecutable(t *testing.T, expectedPath, expectedDigest string) {
	t.Helper()
	...
	if err != nil || executable.Path != expectedPath || executable.Digest != expectedDigest {
		t.Fatalf("actual-host Codex executable identity changed: %+v error=%v", executable, err)
	}
}
